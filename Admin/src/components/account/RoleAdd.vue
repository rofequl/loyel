<template>
  <a-modal
      :title="modalAdd? 'Add new Role':'Role name edit'"
    ...
    return {
      ...
      rules: {
        name: [
          ...
          {max: 50, message: 'Role name maximum length 50 character', trigger: 'change'},
        ]
      }
    };
  },
    ...
    submitForm() {
      this.$refs.ruleForm.validate(valid => {
        if (valid) {
          ...
          this.form.post('role')
              .then(({data}) => {
                this.$store.commit('ROLE_ADD', data);
      ...
      this.$refs.ruleForm.validate(valid => {
        if (valid) {
          ...
          this.form.put('role/' + this.form.id)
              .then(({data}) => {
                this.$store.commit('ROLE_MODIFY', data);
                this.$notification['success']({
                  message: 'Congratulations',
