<template>
  <a-modal
      :title="modalAdd? 'Add new Unit':'Unit information edit'"
      ...
      <a-row :gutter="16">
        <a-col :md="24">
          <a-form-model-item ref="name" has-feedback="" prop="name"
                             label="Unit Name:">
            <a-input v-model="form.name" placeholder="Enter unit name..."
    ...
    return {
      ...
      rules: {
        name: [
          ...
          {max: 50, message: 'Unit name maximum length 50 character', trigger: 'change'},
        ]
      }
    };
  },
    ...
    submitForm() {
      this.$refs.ruleForm.validate(valid => {
        if (valid) {
          ...
          this.form.post('unit')
              .then(({data}) => {
                this.$store.commit('UNIT_ADD', data);
                this.$notification['success']({
                  message: 'Congratulations',
                  description: 'Unit add successfully.',
      ...
      this.$refs.ruleForm.validate(valid => {
        if (valid) {
          ...
          this.form.put('unit/' + this.form.id)
              .then(({data}) => {
                this.$store.commit('UNIT_MODIFY', data);
                this.$notification['success']({
                  message: 'Congratulations',
