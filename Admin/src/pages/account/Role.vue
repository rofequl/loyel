<template>
  <div>
    ...
    <div class="row">
      ...
      <div class="col-12 col-sm-6 d-flex align-items-center">
        <div class="mx-auto ml-sm-auto mr-sm-0">
          <CButton @click="$refs.child.modal()" block size="sm" color="info">Add New Role</CButton>
        </div>
      </div>
    </div>
    <!-- End Page Header -->
    ...
    <div id="people" class="dataTables_wrapper no-footer my-5">
      <v-client-table ref="table" :data="roleList" :columns="columns" :options="options" class="text-center">
        <div slot="serial" slot-scope="props">
          {{ roleIndex(props.row.id) }}
        </div>
...
<script>
import RoleAdd from "@/components/account/RoleAdd";
      ...
      ApiService.delete('role/' + id)
          .then((data) => {
            if (data.data.result === 'Error') {
              ...
              this.$store.commit('ROLE_REMOVE', id);
